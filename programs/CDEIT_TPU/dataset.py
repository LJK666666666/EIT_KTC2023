...
class EITdataset(Dataset):
    ...
    def __init__(self, data_file_path, modelname=None, dataset='simulate', backup_data_path='./data'):
        ...
        self.backup_data_path = backup_data_path  # 备用数据路径，用于加载mean.pth和std.pth
    ...
    def norm(self, ys):
        # 首先尝试从数据文件路径的上级目录加载mean和std
        mean_path = self.data_file_path + '../mean.pth'
        std_path = self.data_file_path + '../std.pth'

        # 如果找不到，则从备用路径加载
        if not os.path.exists(mean_path):
            mean_path = self.backup_data_path + '/mean.pth'
        if not os.path.exists(std_path):
            std_path = self.backup_data_path + '/std.pth'
