...
- Put the data at `./data/` and run with the following commands:
  - **Single GPU:** `python main.py --mode train`
  - **Multi-GPUs:** `accelerate launch main.py --mode train`
  - **TPU:** `accelerate launch --use_tpu main.py --mode train` (requires TPU environment setup)
...
- Put the test data at `./data/`  and run with the following commands:
  - **Single GPU:** `python main.py --mode test --data simulated` for EIDORS generated data, `python main.py --mode test --data uef2017` for UEF2017 dataset or `python main.py --mode test --data ktc2023` for KTC2023 dataset
  - **Multi-GPUs:** `accelerate launch main.py --mode test --data simulated`
  - **TPU:** `accelerate launch --use_tpu main.py --mode test --data simulated` (requires TPU environment setup)
- The prediction will be at  `./results/deit/checkpoints`

## TPU Support

The code now supports training and testing on TPU devices. The Accelerate library automatically detects and handles TPU devices.

### TPU Environment Setup

For Google Cloud TPU or TPU Pod environments:

```bash
pip install torch-xla
accelerate config  # Select TPU as the device
```

### Running on TPU

- **Training:** `accelerate launch --use_tpu main.py --mode train`
- **Testing:** `accelerate launch --use_tpu main.py --mode test --data simulated`
