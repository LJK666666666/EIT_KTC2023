...
def create_method(method_type: str, config: Dict) -> BaseReconstructionMethod:
    ...
    Args:
        method_type: 方法类型 ('cnn', 'diffusion', 'traditional', 'deepdbar', 'fno', 'dbar')
        config: 配置字典
    ...
    elif method_type == 'fno':
        from .fno import create_fno_method
        return create_fno_method(config)

    elif method_type == 'dbar':
        from .dbar import create_dbar_method
        return create_dbar_method(config)
